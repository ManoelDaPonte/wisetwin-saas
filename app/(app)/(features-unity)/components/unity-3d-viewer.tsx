...
export function Unity3DViewer({
  ...
  const handleTrainingCompleted = useCallback(
    async (jsonDataString: string) => {
      ...
      if (!buildName || !buildType || !containerId) {
        console.error("Missing required parameters for training completion");
        return;
      }
      ...
      try {
        ...
        console.log("Training completion received from Unity:", analyticsData);
        ...
        if (response.ok) {
          ...
          console.log("Training completion processed successfully:", result);
  ...
  useEffect(() => {
    ...
    console.log("Setting up Unity event listener for training completion...");
