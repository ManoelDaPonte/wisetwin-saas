...
import { useMemberCompletions } from "../hooks/use-member-completions";
...
export function ProgressDashboard({}: ProgressDashboardProps) {
	...
	// Hook pour obtenir les formations terminées par tous les membres de l'organisation
	const { data: memberCompletionsData } = useMemberCompletions("WISETRAINER");
	const memberCompletions = memberCompletionsData?.memberCompletions || [];
	...
	const isFormationCompletedByMember = (
		...
		const isCompleted = memberCompletions.some(
			(completion) =>
				completion.userId === memberId &&
				(completion.buildName === buildName ||
					completion.buildName === buildName.replace(/\s+/g, "")) // Au cas où il y aurait des différences d'espaces
		);
							...
							<div className="ml-4">
								...
								<div className="flex items-center gap-2">
									...
									<Badge
										...
										className="text-xs"
									>
										{dashboardMetrics?.tagsWithMembers || 0}{" "}
										en cours
									</Badge>
								</div>
							</div>
					...
					<CardHeader>
						<div className="flex items-center justify-between">
							<div>
								...
								<CardDescription>
									Cliquez sur un plan pour voir les détails et
									la progression
								</CardDescription>
							</div>
										...
										<CardContent className="p-6">
											<div className="flex items-center">
												...
												<div className="ml-4">
													...
													<div className="flex items-center gap-2">
														<p className="text-2xl font-bold">
															{/* Pour l'instant 0 car pas encore de système de completion des formations */}
															0
														</p>
														<Badge
															variant="outline"
																	...
																	<div className="text-sm">
																		{(() => {
																			const assignedBuilds =
																				...
																				[];
																			const totalBuilds =
																				assignedBuilds.length;
																			const completedBuilds = 0; // TODO: Implémenter le système de completion
																			const percentage =
																				totalBuilds >
																				0
																					? Math.round(
