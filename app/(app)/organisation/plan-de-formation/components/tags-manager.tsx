...
export function TagsManager({}: TagsManagerProps) {
  ...
  const { tagsWithStats } = useTrainingDashboard();
  ...
  const handleEditTag = (updatedData: UpdateTrainingTagData) => {
    if (tagToEdit) {
      updateTag(
        { tagId: tagToEdit.id, data: updatedData },
        {
          onSuccess: () => {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base">{t.tagsManager.title}</CardTitle>
              <CardDescription>{t.tagsManager.subtitle}</CardDescription>
            </div>
                    ...
                    <TableRow key={tag.id} className={tag.archived ? "opacity-60" : undefined}>
                      ...
                      <TableCell className="text-center">
                        {(() => {
                          ...
                          const membersCompleted = Math.floor(
                            stats.totalCompletions / stats.buildCount
                          );
